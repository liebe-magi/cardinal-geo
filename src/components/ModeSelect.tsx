...
export function ModeSelect() {
  ...
  const appConfigCache = useRef<Awaited<ReturnType<typeof fetchAppConfig>> | null>(null);
  ...
  const handleStart = async (mode: GameMode, subMode: GameSubMode) => {
    ...
    // Version gate: block game start if client is outdated
    // Use cached config, but re-fetch to ensure freshness
    const config = await fetchAppConfig();
    appConfigCache.current = config;
    if (config.maintenance) {
      setIsMaintenance(true);
      ...
      {isMaintenance && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-gray-800 border border-amber-500/40 rounded-2xl shadow-xl shadow-amber-500/10 px-6 py-5 mx-4 max-w-sm w-full text-center">
            ...
            <h3 className="text-lg font-bold text-white mb-2">メンテナンス中</h3>
      ...
      {showUpdateRequired && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-gray-800 border border-cyan-600/40 rounded-2xl shadow-xl shadow-cyan-500/10 px-6 py-5 mx-4 max-w-sm w-full text-center">
            <div className="text-3xl mb-3">🔄</div>
            <h3 className="text-lg font-bold text-white mb-2">アップデートが必要です</h3>
            <p className="text-sm text-gray-300 mb-4">
              新しいバージョンが公開されています。
              <br />
              ページを更新してからプレイしてください。
